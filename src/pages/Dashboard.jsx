...
import { getDashboardSummary, getApplications, getErrorMessage } from "../api";
    ...
    Promise.all([getDashboardSummary(), getApplications("", 100)])
      ...
      .catch((err) => {
        if (err.response?.status === 401 || err.response?.status === 403) {
          ...
          setError(getErrorMessage(err, "Failed to load dashboard. Please check your connection or log in again."));
        }
      })
  ...
  if (error) {
    return (
      <div className="page" style={{ textAlign: "center", paddingTop: "40px" }}>
        ...
        <button className="btn btn-primary" onClick={() => { localStorage.removeItem("token"); navigate("/login"); }}>
          Go to Login
        </button>
      </div>
    );
                    ...
                    >
                      ...
                      <td>
                       ...
                       <td className="date-cell">
                          {app.date_applied ? parseDateOnly(app.date_applied).toLocaleDateString() : "—"}
                        </td>
                    </tr>
                  );
                })}
